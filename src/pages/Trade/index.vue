  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div
        ...
        :key="item.id"
        @click="changeDefault(item, userAddress)"
      >
      ...
      <div class="detail">
        ...
        <ul
          class="list clearFix"
          v-for="detail in tradeDetail.detailArrayList"
          :key="detail.sourceId"
        >
          <li>
            <img :src="detail.imgUrl" alt="" style="width:100px;height:60px" />
          </li>
          <li>
            <p>
              {{ detail.skuName }}
    ...
    <div class="money clearFix">
      <ul>
        <li>
          <b
            ><i>{{ tradeDetail.totalNum }}</i
            >件商品，总商品金额</b
          >
          <span>¥{{ tradeDetail.totalAmount }}</span>
        </li>
        <li>
          <b>返现：</b>
          <span>0.00</span>
    ...
    <div class="trade">
      <div class="price">
        应付金额: <span>¥{{ tradeDetail.totalAmount }}</span>
      </div>
...
export default {
  ...
  methods: {
    async getGoodsTrade() {
      const result = await this.$API.reqGoodsTrader();
      if (result.code === 200) {
        this.tradeDetail = result.data;
      }
    },
    async getUserAddresses() {
      const result = await this.$API.reqUserAddresses();
      if (result.code === 200) {
    ...
    async submitOrder() {
      let tradeNo = this.tradeDetail.tradeNo;
      let tradeInfo = {
        ...
        orderDetailList: this.tradeDetail.detailArrayList,
      };
